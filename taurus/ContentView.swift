 ...
 */
    ...
    private func parsedContent() -> [Chunk] {
        var chunks: [Chunk] = [];
        for line in self.content.split(separator: "\n") {
            let l = String(line)
            if l.hasPrefix("=>") {
                let parts = l.split(separator: " ")
                let url = parts[1];
                let description = parts[2...].map { String($0) }.joined(separator: " ");
                chunks.append(Chunk(type: "link", url: String(url), description: String(description)))
            } else {
                chunks.append(Chunk(type: "text", description: l));
            }
        }
        return chunks;
    }
